...
// ── Step 2: Read coffee table ──
const tableDoc = await io.read('public/models/coffee_table_final (1).glb');
...
for (const mesh of tableRoot.listMeshes()) {
  for (const prim of mesh.listPrimitives()) {
    ...
    if (!prim.getAttribute('TEXCOORD_0')) {
      ...
      const uvAccessor = tableDoc.createAccessor('texcoord_0')
        .setType('VEC2')
        .setArray(uvData);
      prim.setAttribute('TEXCOORD_0', uvAccessor);
      console.log(`  Generated TEXCOORD_0 for ${count} vertices (XZ box projection)`);
    }
    ...
    if (!prim.getAttribute('NORMAL')) {
      ...
      const normalAccessor = tableDoc.createAccessor('normal')
        .setType('VEC3')
        .setArray(normals);
...
const newTexture = tableDoc.createTexture('walnut_wood_texture')
...
await io.write('public/models/coffee_table_final (1).glb', tableDoc);
