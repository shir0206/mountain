...
// ── Step 1: Read table and extract the WALNUT material ──
const tableDoc = await io.read(
...
if (!walnutMat) throw new Error("Could not find WALNUT material in table");
...
console.log(`Done – ${updated} material(s) now use table WALNUT wood texture.`);
